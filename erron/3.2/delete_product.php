<?php
    ...
    foreach ($dataRow as $value) {
        $sqlgetProduct = "SELECT image FROM products WHERE name = '{$value[1]}' AND brand = '{$value[2]}' AND type = '{$value[3]}' AND quantity = '{$value[4]}' AND price = '{$value[5]}'";
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "DELETE FROM products WHERE name = '{$value[1]}' AND brand = '{$value[2]}' AND type = '{$value[3]}' AND quantity = '{$value[4]}' AND price = '{$value[5]}'";
            if ($conn->query($sql)) {
                $success = true;
            } 
        } 
